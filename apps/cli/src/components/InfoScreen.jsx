...
// `duration` ms, z widocznym odliczeniem, i pozwalamy pominąć czas DOWOLNĄ
// akcją (klawisz). `onDismiss` woła się raz — czy to z timeoutu, czy z klawisza.
export default function InfoScreen({ title, message, duration = 4000, onDismiss, color = 'green', t }) {
  ...
  return (
    <Box flexDirection="column" borderStyle="round" borderColor={color} paddingX={1}>
      ...
      <Text>{message}</Text>
      <Text dimColor>{tfmt(t.InfoAnyKeyClose, { seconds: secondsLeft })}</Text>
    </Box>
  );
}
