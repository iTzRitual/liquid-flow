...
/** The onboarding split layout: a marketing column on the left (brand, tagline,
 * feature list) and a form region on the right. Pure structural layout. The
 * marketing column is a shrink-to-fit half that hides below `md`, so on narrow
 * windows the form takes the full width. The caller fills both slots. */
  ...
  return (
    <div className={cn('flex h-full overflow-hidden bg-surface-app', className)}>
      ...
      <div className="min-w-0 flex-1 overflow-hidden p-2">
        <div className="flex h-full items-center justify-center overflow-y-auto">{children}</div>
      </div>
    </div>
  );
}
